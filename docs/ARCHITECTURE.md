...
- **SigningKey** — key pair (PEM-encoded) tagged with algorithm (RS256 or EdDSA), kid in JWT headers. Three-state lifecycle: active (signs and verifies, in JWKS), deactivated (verifies and stays in JWKS but stops signing), deleted (gone entirely). A realm may have multiple active keys across algorithms simultaneously; the token endpoint prefers RS256 for signing when both are present, and verification dispatches on the token's `kid` header against all stored keys.
